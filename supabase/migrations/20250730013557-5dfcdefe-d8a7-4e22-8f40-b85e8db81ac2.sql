...
-- FASE 1: Limpeza e Manutenção Urgente

-- Executar VACUUM e ANALYZE em todas as tabelas críticas
VACUUM ANALYZE public.products;
VACUUM ANALYZE public.orders;
VACUUM ANALYZE public.profiles;
VACUUM ANALYZE public.marketplace_products;
VACUUM ANALYZE public.api_connections;
VACUUM ANALYZE public.sync_executions;

-- FASE 2: Criação de Índices Compostos Críticos
...
ON public.orders (status, created_at DESC) WHERE status IN ('pending', 'completed', 'cancelled');

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_status_login 
ON public.profiles (status, last_login DESC) WHERE status IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_created_status 
ON public.profiles (created_at DESC, status) WHERE created_at >= CURRENT_DATE - INTERVAL '30 days';

-- Índices para produtos e marketplace
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_active_featured 
ON public.products (active, featured, created_at DESC) WHERE active = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_category_active 
ON public.products (category_id, active, price) WHERE active = true;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_marketplace_products_sync 
ON public.marketplace_products (api_connection_id, sync_status, last_sync_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_marketplace_products_import 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sync_executions_connection_date 
...
-- FASE 3: Otimização da função fetch_categories_with_subcategories
...
-- FASE 4: Criar função otimizada para is_admin com cache
...
-- FASE 5: Configurar autovacuum otimizado para tabelas críticas

-- Configurar autovacuum para tabelas com alta rotatividade
ALTER TABLE public.marketplace_products SET (
  autovacuum_vacuum_scale_factor = 0.1,
  autovacuum_analyze_scale_factor = 0.05,
  autovacuum_vacuum_cost_delay = 10
);

ALTER TABLE public.sync_executions SET (
  autovacuum_vacuum_scale_factor = 0.1,
  autovacuum_analyze_scale_factor = 0.05
);
...
-- Configurar autovacuum para tabelas de produtos
ALTER TABLE public.products SET (
  autovacuum_analyze_scale_factor = 0.02
);

-- FASE 6: Criar função para limpeza automática de logs antigos
