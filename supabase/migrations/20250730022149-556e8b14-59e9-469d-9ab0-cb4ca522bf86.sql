...
-- Implementação completa das 5 fases de otimização

-- ================================================
-- FASE 1: LIMPEZA CRÍTICA IMEDIATA
-- ================================================

-- VACUUM FULL para limpeza agressiva de tuplas mortas
VACUUM (FULL, ANALYZE) public.products;
VACUUM (FULL, ANALYZE) public.categories;
VACUUM (FULL, ANALYZE) public.subcategories;
VACUUM (FULL, ANALYZE) public.profiles;
VACUUM (FULL, ANALYZE) public.orders;
VACUUM (FULL, ANALYZE) public.marketplace_products;
VACUUM (FULL, ANALYZE) public.api_connections;
VACUUM (FULL, ANALYZE) public.user_activity_logs;
VACUUM (FULL, ANALYZE) public.api_metrics;

-- REINDEX para reorganizar índices fragmentados
REINDEX TABLE public.products;
REINDEX TABLE public.categories;
REINDEX TABLE public.subcategories;
REINDEX TABLE public.profiles;
REINDEX TABLE public.orders;
REINDEX TABLE public.marketplace_products;

-- ================================================
-- FASE 2: ÍNDICES OTIMIZADOS PARA QUERIES COMUNS
-- ================================================
...
-- ================================================
-- FASE 3: FUNÇÕES DE MONITORAMENTO E ALERTAS
-- ================================================
...
-- ================================================
-- FASE 4: SISTEMA DE ALERTAS AUTOMÁTICOS
-- ================================================
...
BEGIN
  -- Auto-vacuum em tabelas que precisam
  FOR table_record IN 
    SELECT table_name FROM public.monitor_table_health() 
    WHERE needs_vacuum = true
  LOOP
    EXECUTE format('VACUUM ANALYZE %I', table_record.table_name);
