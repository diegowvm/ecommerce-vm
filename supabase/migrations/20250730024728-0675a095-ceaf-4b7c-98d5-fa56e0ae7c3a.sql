...
-- Implementação com nomes corretos das colunas
...
AS $$
  WITH stats AS (
    SELECT 
      ...
      pg_size_pretty(SUM(pg_total_relation_size(schemaname||'.'||relname))) as total_db_size
...
INSERT INTO public.system_alerts (
  ...
  'system',
  'info',
  'Performance Optimization System Installed',
  'Sistema completo de monitoramento e otimização de performance implementado. Execute VACUUM manualmente no SQL Editor para completar a otimização.',
  jsonb_build_object(
    'phases_completed', 4,
    'features', ARRAY[
      'performance_indexes_created', 
      'monitoring_functions_installed',
      'automated_alerts_active',
      'maintenance_system_ready'
    ],
    'next_steps', ARRAY[
      'Execute VACUUM ANALYZE em todas as tabelas',
      'Configure alertas automáticos',
      'Execute public.automated_maintenance() periodicamente'
    ]
  )
);