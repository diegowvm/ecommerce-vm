...
CREATE INDEX IF NOT EXISTS idx_profiles_user_id_status ON public.profiles(user_id, status) WHERE status = 'active';
...
CREATE INDEX IF NOT EXISTS idx_marketplace_products_connection_sync ON public.marketplace_products(api_connection_id, sync_status);
CREATE INDEX IF NOT EXISTS idx_marketplace_products_imported_active ON public.marketplace_products(is_imported, auto_sync_enabled) WHERE is_imported = true;
CREATE INDEX IF NOT EXISTS idx_sync_executions_connection_status ON public.sync_executions(api_connection_id, status);
CREATE INDEX IF NOT EXISTS idx_sync_executions_started_status ON public.sync_executions(started_at DESC, status);
...
CREATE INDEX IF NOT EXISTS idx_api_metrics_marketplace_created ON public.api_metrics(marketplace_name, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_cart_items_user_updated ON public.cart_items(user_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_wishlist_user_created ON public.wishlist(user_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_system_alerts_type_severity ON public.system_alerts(alert_type, severity) WHERE NOT is_resolved;
