...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  const { metrics, imageMetrics, performanceGrade } = usePerformanceMetrics();
  ...
  const getGradeColor = (grade: string) => {
    switch (grade) {
      ...
      case 'needs-improvement': return 'bg-yellow-500/10 text-yellow-700 border-yellow-200';
    ...
    return (
      <div className={`fixed bottom-4 right-4 z-50 ${className}`}>
        <Button
          size="sm"
          variant="outline"
          className={`${getGradeColor(performanceGrade)} backdrop-blur-sm`}
          onClick={() => setIsExpanded(!isExpanded)}
        >
          {getGradeIcon(performanceGrade)}
          <span className="ml-2 text-xs font-medium">
            Performance: {performanceGrade === 'good' ? 'Boa' : 
                        performanceGrade === 'needs-improvement' ? 'Moderada' : 'Ruim'}
          </span>
        </Button>
        
        {isExpanded && (
          <Card className="absolute bottom-12 right-0 w-80 shadow-lg backdrop-blur-sm bg-background/95">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="border-t pt-2">
                ...
                {imageMetrics.averageLoadTime > 0 && (
    ...
    <div className={`space-y-4 ${className}`}>
      <div className="flex items-center gap-2">
        ...
        <Badge className={getGradeColor(performanceGrade)}>
          {getGradeIcon(performanceGrade)}
          <span className="ml-1">
            {performanceGrade === 'good' ? 'Excelente' : 
             performanceGrade === 'needs-improvement' ? 'Moderado' : 'Crítico'}
          </span>
        </Badge>
      </div>
        ...
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <div className="font-mono font-semibold">
              {imageMetrics.totalImages > 0 
                ? Math.round((imageMetrics.loadedImages / imageMetrics.totalImages) * 100)
                : 0}%
            </div>
          </div>
        </CardContent>
